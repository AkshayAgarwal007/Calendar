...
TTimeEdit::TTimeEdit(const char* name, uint32 sections,
	BMessage* message)
	:
	TSectionEdit(name, sections, message),
	fLastKeyDownTime(0),
	fFields(NULL),
	fFieldCount(0),
...
void
TTimeEdit::SetDate(const BDate& date)
{
	fTime.SetDate(date);
}
...
void
TTimeEdit::SetTime_t(time_t time)
{
	fTime.SetTime_t(time);
...
TTimeEdit::_UpdateFields()
{
	...
	fTimeFormat.GetTimeFields(fFields, fFieldCount, B_SHORT_TIME_FORMAT);
}
...
{
	SetDate(BDate(year, month, day));
}


void
TDateEdit::SetDate(const BDate& date)
{
	fDate = date;
